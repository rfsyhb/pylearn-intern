"""
Hasilnya adalah Palangkaraya Terlebih dahulu, kemudian Sampit.
"""
                ...
                try:
                    ...
                    for entry in data["data"]:
                        if entry["name"] == "Kota Palangkaraya":
                            ...
                            if harga:  # Check if harga is not empty
                                harga = f"{float(harga):.0f}"  # Format harga as integer without decimals
                            ...
                            output_data.append({
                                "TANGGAL": current_date.strftime("%d/%m/%Y"),  # only for debugging
                                "KOTA_KAB": entry["name"],
                                "CAT_ID": description,  # only for debugging
                                "PRICE_TYPE": priceTypeDesc,  # only for debugging
                                "HARGA_SAT": harga
                            })
...
output_data.sort(key=lambda x: (x['KOTA_KAB'], x['TANGGAL'])) # Jika kota sama, urutkan berdasarkan tanggal

# Format output untuk clipboard
header = "KOTA_KAB\tHARGA_SAT"
output_string = header + "\n" + "\n".join([f"{item['KOTA_KAB']}\t{item['HARGA_SAT']}" for item in output_data])
